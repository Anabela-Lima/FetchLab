...
function getCountryByName(country)  {
    ...
    let endpoint1= `https://restcountries.com/v3.1/name/${country}/` ;   // here i am using string interpolation (i.e. `` and the curly braces + dollar sign ) so i can pass the function argument in the url - using the country in url
    
    fetchCountry =  fetch(endpoint1);  // fetch takes the variable endpoint1 as input argument
    ...
    fetchCountry.then(response => {       // we fetch the country with endpoint, then we get a response from it
        if(response.ok){                    // if response is ok, i,e program ran smoothly, 
           ...
           section1= document.querySelector("#section1")  // i also wanted to add another section with country facts so i added a section in html with id=section1, i then store this inside variable section1
           header.innerHTML = "🟢 loaded  "
            
           section1.innerHTML = `Country name is ${data[0].name.official} and its population is ${data[0].population} <br></br>` // call innerHTML function on section 1 to change its text, and I equate it to a special type of string = "string interpolation"- which allows me then to use variables inside it note: data[0] is the object, i then get hold of its specific properties
            })
        
        } else {header.innerHTML = `Error 404 on country: ${country}` + alert("please provide a valid country!")} // if the above conditions fails then I want header element to change to error etc, and a popup window to appear
    })
...
clearButton= document.querySelector("#clear") // clear button is element in our html document with id="clear"
...
const doubleClickFunction= (e) => {                   // doubleClickFunction= function takes an event argument 
    ...
    clearingList= document.querySelector("#myCountriesList") 
...
function clearButtonSpeak() {           // we have a function called clearButtonSpeak- note this is not really the clear button speaking- I added a section that will come up with text below the clear button
    ...
    divMouse= document.querySelector("#approach-clear-button")  // section = the one with id approach-Clear-button and i store this in some variable "divMouse"
...
changeColour= () => {                                // change colours is a function that takes no input arguments
    const body = document.querySelector("#body");   // scan document for element with id = body and store it in a constant body variable
   for(item in colours){                            // looping through the array "colours"
...
addP=() =>{ return paragraph = document.createElement('p')}    // this function creates a p element in our document. It is then stored in a variable "paragraph"

// countries section
countriesSection= document.querySelector("#allCountries")  // countries section is the element in our document with an id of "allCountries"
...
function getAllCountries(){                         // get all countries is function 
    ...
    let endpoint2= "https://restcountries.com/v3/all" ;  // storing the link in endpoint2
    
    fetchCountry =  fetch(endpoint2);  // fetch takes the variable endpoint as input argument and this result is sotred in fetchCountry argyment
    ...
    fetchCountry.then(response => {       // we fetch the country by calling our fetchCountry argument, then we get a response from it
        if(response.ok){                    // if response is ok, i,e program ran smoothly, 
            ...
            paragraph.innerHTML= ` 📍  Country name is ${data[c].name.common} and its population is ${data[c].population} <br></br>` // we then set the p element or "paragraph" to have an inner text of  Country name is ${data[c].name.common} and its population is ${data[c].population} <br></br>` Note, data[c].name.common- im just getting the data for the element we are currently at and getting propoerty name, common
